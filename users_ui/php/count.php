...
$check2 = "SELECT * FROM product_post WHERE user_seller_id = '$user_id' AND prod_id = '$id' ";
	...
	if ( $row2 > 0 ) {
	echo "you'cannot liked on your own product  ";
...
if ( $row > 0 ) {
	echo "you already rated this post ";
			...
			$comm = "INSERT INTO product_likes(user_id,prod_id,rating,notif) VALUES('$user_id','$id','$rating','$notif') ";
			$res_comm = mysqli_query($connect, $comm);
			
			if ($res_comm) {
				echo "yes";

					 $sql8 = "INSERT INTO activity_log values('','$u_name','$user_id',CURRENT_TIMESTAMP,'rated a Photo $rating star','0')";
                  	 $result8 = mysqli_query($connect,$sql8); 

                  	  $sql_n = "INSERT INTO notifications values('','$user_id','$id','rated your photo $rating star',CURRENT_TIMESTAMP,'0')";
